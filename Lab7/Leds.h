 ...
 */
#define LEDS_INIT() do { \
    TRISECLR = 0xFF;     /* Sets TRIS register bits for LEDs as output */ \
    LATECLR = 0xFF;      /* Clears the LATE register bits to turn off LEDs */ \
} while (0)
 ...
 */
#define LEDS_SET(newPattern) do { \
    LATECLR = 0xFF;      /* Clears the LEDs */ \
    LATESET = (newPattern & 0xFF); /* Set to newPattern */ \
} while (0)
 ...
 */
#define LEDS_GET() (LATE & 0xFF)  /* Returning current LED state */
